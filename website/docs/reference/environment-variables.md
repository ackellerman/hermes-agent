...
| `HERMES_KANBAN_BOARD` | Pin the active kanban board for this process. **Interactive sessions** resolve the env board only when `kanban.env_board_pin` is `true` (default off); under the default off the inherited env is skipped and resolution is project/config driven instead (see `cli-commands.md`). **Workers and cron children** (`HERMES_KANBAN_DB` set) always honor it — the dispatcher injects it so workers physically cannot see tasks on other boards. Defaults to `default`. Slug validation: lowercase alphanumerics + hyphens + underscores, 1-64 chars |
