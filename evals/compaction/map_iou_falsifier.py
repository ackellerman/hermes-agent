...
- AC-3 (IoU): drives 10 consecutive ``CompactionMap.update`` calls over the
  ...
  the update calls through a real model at the explicit ``--ollama`` endpoint.
...
def run(*, map_model: str = "deterministic", padding_tokens: int = 0,
    ...
    if map_model == "real":
        if not ollama_endpoint:
            raise ValueError("--ollama is required when --map-model=real")
...
def main() -> int:
    ...
    parser.add_argument("--ollama", help="endpoint required with --map-model=real")
