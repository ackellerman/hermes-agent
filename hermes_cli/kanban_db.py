...
def block_task(
    ...
    **A dependency is an edge or it does not exist** (overlay, 2026-09-04):
    ``kind='dependency'`` REQUIRES ``depends_on`` naming the task ids being
    waited on; the kernel writes them as ``task_links`` parents in the same
    transaction. Without the edge, ``recompute_ready`` sees ``all([])`` and
    re-promotes the card on every dispatcher tick — one card was cold-spawned
    97 times in 2.4 h waiting on a parent that existed only in its block
    reason. Refused with :class:`ValueError` when ``depends_on`` is
    missing/empty, names an unknown task, names a task that is already
    done/archived (nothing to wait on), or would create a cycle.
    """
    ...
    with write_txn(conn):
        ...
        if kind == "dependency":
            ...
            missing = _missing_task_ids(conn, dep_ids)
            if missing:
                raise ValueError(f"unknown task(s): {', '.join(missing)}")
            for dep in dep_ids:
