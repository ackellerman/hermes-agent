...
def test_dependency_then_parent_done_promotes(kanban_home: Path) -> None:
    ...
    with kbc.connect_closing() as conn:
        ...
        # Overlay K1: a dependency block may name its parents as an optional
        # enhancement (the edge already exists here; INSERT OR IGNORE keeps it
        # idempotent). No hard refusal when absent — upstream's re-kind owns
        # the no-parent shape.
